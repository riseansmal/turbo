...
use crate::chunk::content::EcmascriptChunkContentReadRef;
...
pub(super) struct EcmascriptMergedChunkContent {
    pub contents: Vec<EcmascriptChunkContentReadRef>,
}
        ...
        Ok(EcmascriptMergedChunkVersion {
            versions: self
                ...
                .map(|content| async move {
                    let content = ReadRef::cell(content.clone());
                    content.version().await
                })
                .try_join()
                .await?,
        }
