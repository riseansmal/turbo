...
use serde::Serialize;
use turbo_tasks::{primitives::JsonValueVc, ReadRef};
...
use super::{content::EcmascriptMergedChunkContentVc, version::EcmascriptMergedChunkVersionVc};
use crate::chunk::{
    update::{update_ecmascript_chunk, EcmascriptChunkUpdate},
    version::{EcmascriptChunkVersionReadRef, EcmascriptChunkVersionVc},
};
...
struct MergedModuleMap {
    versions: Vec<EcmascriptChunkVersionReadRef>,
}

impl MergedModuleMap {
    /// Creates a new `MergedModuleMap` from the given versions.
    fn new(versions: Vec<EcmascriptChunkVersionReadRef>) -> Self {
        Self { versions }
    }
    ...
    fn get(&self, id: &ModuleId) -> Option<u64> {
        for version in &self.versions {
            if let Some(hash) = version.module_factories_hashes.get(id) {
                return Some(*hash);
            }
        }
        None
...
pub(super) async fn update_ecmascript_merged_chunk(
    content: EcmascriptMergedChunkContentVc,
    from_version: VersionVc,
) -> Result<Update> {
    let to_merged_version = content.version();
    let from_merged_version =
        if let Some(from) = EcmascriptMergedChunkVersionVc::resolve_from(from_version).await? {
            from
        } else {
    ...
    let mut from_versions = Vec::with_capacity(from.versions.len());
    let mut from_versions_by_chunk_path = IndexMap::with_capacity(from.versions.len());

    for version_vc in &from.versions {
        let Some(version_vc) = EcmascriptChunkVersionVc::resolve_from(ReadRef::cell(version_vc.clone())).await? else {
                bail!("version is not an EcmascriptChunkVersionVc")
            };

        let version_ref = version_vc.await?;
        from_versions.push((version_ref, version_vc));
    }

    for (version_ref, version_vc) in &from_versions {
        if from_versions_by_chunk_path
            .insert(&*version_ref.chunk_server_path, *version_vc)
            .is_some()
        {
            bail!(
                "trying to merge multiple versions for the same chunk {}",
                version_ref.chunk_server_path
            );
        }
    }

    let merged_module_map =
        MergedModuleMap::new(from_versions.iter().map(|(v, _)| v.clone()).collect());

    let content = content.await?;
    let to_contents = content.contents.iter();

    let mut merged_update = EcmascriptMergedUpdate::default();

    for content_ref in to_contents {
        let content = ReadRef::cell(content_ref.clone());
        ...
        let chunk_update =
            if let Some(from_version) = from_versions_by_chunk_path.remove(chunk_server_path) {
                ...
                match &*update {
                    ...
                    EcmascriptChunkUpdate::Partial(chunk_partial) => {
                        ...
                        partial
                            .deleted
                            .extend(chunk_partial.deleted.keys().cloned());
        ...
        merged_update.chunks.insert(chunk_server_path, chunk_update);
    }

    // Deleted chunks.
    for (chunk_server_path, chunk_version) in from_versions_by_chunk_path {
        let hashes = &chunk_version.await?.module_factories_hashes;
